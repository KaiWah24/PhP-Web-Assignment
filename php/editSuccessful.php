<?php
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST'){
        $eventName = ($_POST['NewName']);
        $NewGame = ($_POST['editGame']);
        $NewVenue =($_POST['editVenue']);
        $NewDate = ($_POST['editDate']);
        $NewPrice = ($_POST['editPrice']);
        $NewDescription = ($_POST['editDescription']);
    }

        $update = "UPDATE event SET GAME_NAME = ?, event_venue = ?, EVENT_DATE = ?, PRICE_POOL = ?, event_DESCRIPTION = ? WHERE EVENT_NAME = ?";
        $stmt = $dbc->prepare($update);
        $stmt -> bind_param('sssdss',$NewGame,$NewVenue,$NewDate,$NewPrice,$NewDescription,$eventName);
        $stmt -> execute();
...
<html>
    ...
    <body>
        <h2 style="color: #66fcf1; margin-left:200px;">Edit Successful</h2>
        <hr>
        ...
        <table class='table table-hover table-responsive' cellpadding='5' style="color:#c5c6c7;">                
            ...
            <tr>
                <td>
                    Price Pool: RM <?php echo $NewPrice ?>
                </td>
            </tr>                 
            <!-- END PRICE POOL -->
            ...
            <tr>
                <td>
                    Description: <?php echo $NewDescription ?>
                </td>
            </tr>                   
            <!-- END DESCRIPTION-->  
